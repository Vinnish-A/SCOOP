...
2. `02_core`：FastCore-backed normalization、HVG、PCA、Harmony 2.0、kNN、UMAP、Leiden sweep；`scanpy_legacy` 是唯一 fallback。
...
# 2. 复制 run 模板到本地工作区
mkdir -p .scoop_local/runs
cp -r templates/run .scoop_local/runs/2026-06-12_001

# 3. 编辑配置
vim .scoop_local/runs/2026-06-12_001/config/run.yaml

# 4. 运行 QC
python scripts/01_qc_scrublet.py \
  --config .scoop_local/runs/2026-06-12_001/config/run.yaml
...
python scripts/04d_annotation_commit.py \
  --config .scoop_local/runs/2026-06-12_001/config/run.yaml \
  --decisions .scoop_local/runs/2026-06-12_001/04_annotation/decisions/annotation_decision_template.json
...
python scripts/08_prepare_pseudobulk.py \
  --config .scoop_local/runs/2026-06-12_001/config/run.yaml
fastde deseq2 .scoop_local/runs/2026-06-12_001/07_de/pseudobulk/<cell_type> condition ctrl test
...
- `templates/run/`：run 目录模板，不保存真实运行输出。
- `.scoop_local/data/`：本地测试数据区，被 git 忽略。
- `.scoop_local/runs/`：本地测试输出区，被 git 忽略。
...
- `scripts/`：Agent 可直接调用的命令行脚本；R 参考脚本放在 `scripts/r/`。
