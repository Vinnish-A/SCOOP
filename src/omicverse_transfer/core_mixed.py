...
def run_omicverse_mixed_core(adata, cfg: Mapping[str, Any], run_root: str | Path) -> dict[str, Any]:
    ...
    options = build_options(cfg, adata, backend="omicverse_cpu_gpu_mixed", mode="cpu-gpu-mixed")
    ...
    artifacts = write_core_tables(adata, run_root, method="omicverse_cpu_gpu_mixed", options=options)
    return summarize_result(
        ...
        artifacts=artifacts,
        quality_basis="omicverse_cpu_gpu_mixed_adapter_smoke",
    )
