...
def leiden_sweep(adata, graph_prefix: str = "identity", resolutions: Iterable[float] = (0.4, 0.8, 1.2), seeds: Iterable[int] = (0, 1, 2, 3, 4)) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for res in resolutions:
        ...
        for seed in seeds:
            ...
            sweep_rows.append({"resolution": float(res), "seed": int(seed), "n_clusters": int(pd.Series(labels).nunique())})
    stability_rows = []
    ...
    stability = pd.DataFrame(stability_rows).sort_values(["median_ari", "resolution"], ascending=[False, True])
    # Conceptual default: choose lowest resolution with adequate seed stability.
    candidates = stability[stability["median_ari"] >= 0.8]
    chosen = float(candidates["resolution"].min()) if len(candidates) else float(stability.iloc[0]["resolution"])
