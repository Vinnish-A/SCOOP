...
TMP_DIR="$ROOT/tmp/cnmf_spatial_batch_adjusted_benchmark"
INPUT="$TMP_DIR/input/gbm_lowres_visium_3samples_sample_batch_adjusted.h5ad"
CNMF="$ROOT/.venv-cnmf/bin/cnmf"
TIME="/usr/bin/time"
NAME="gbm_lowres_sample_batch_adjusted_cnmf"
K_VALUES=(6 8)
N_ITER=8
MAX_NMF_ITER=200
SEED=20260614
...
if [[ ! -f "$INPUT" ]]; then
  echo "Missing input H5AD: $INPUT" >&2
  exit 1
fi
...
run_serial() {
  ...
  run_timed prepare "$log_dir" "$CNMF" prepare \
    --output-dir "$out_dir" --name "$NAME" --counts "$INPUT" \
