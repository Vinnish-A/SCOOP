...
from fastde.abundance_design import build_covariate_matrix, encode_multiclass_labels
...
def test_multiclass_design_reference_level() -> None:
    ...
    design = build_covariate_matrix(table.metadata, ["batch"])
    assert classes[ref] == "control"
    assert list(keep) == ["S1", "S2", "S3"]
    assert design.shape[0] == 3
    assert "batch_y" in design.columns
